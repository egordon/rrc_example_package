...
_cuboid_3d_radius = np.sqrt(0.5 * _CUBOID_WIDTH * _CUBOID_WIDTH + 0.25 * _CUBOID_HEIGHT * CUBOID_HEIGHT)
_max_cuboid_com_distance_to_center = _ARENA_RADIUS - _cuboid_3d_radius

_min_height = _CUBOID_HEIGHT / 2
_max_height = 0.1


_cuboid_corners = np.array(
    [
        [-1 * (_CUBOID_HEIGHT / _CUBOID_WIDTH), -1, -1],
        [-1 * (_CUBOID_HEIGHT / _CUBOID_WIDTH), -1, +1],
        [-1 * (_CUBOID_HEIGHT / _CUBOID_WIDTH), +1, -1],
        [-1 * (_CUBOID_HEIGHT / _CUBOID_WIDTH), +1, +1],
        [+1 * (_CUBOID_HEIGHT / _CUBOID_WIDTH), -1, -1],
...
def get_cuboid_corner_positions(pose):
    ...
    return rotation.apply(_cuboid_corners) + translation
...
def sample_goal(difficulty):
    ...
    def random_xy():
        ...
        radius = _max_cuboid_com_distance_to_center * np.sqrt(random.random())
    ...
    if difficulty == -1:  # for initialization
        ...
        z = random.uniform(_cuboid_3d_radius, _max_height)
...
def validate_goal(goal):
    ...
    if np.linalg.norm(goal.position[:2]) > _max_cuboid_com_distance_to_center:
    ...
    corners = get_cuboid_corner_positions(goal)
...
def evaluate_state(goal_pose, actual_pose, difficulty):
    ...
    if difficulty in (1, 2, 3):
        ...
        # goal_corners = get_cuboid_corner_positions(goal_pose)
        # actual_corners = get_cuboid_corner_positions(actual_pose)
